...
async function executeQuery() {
  ...
  try {
    const result = await sql.query("SELECT TOP 5 * FROM AllDatas");
...
executeQuery().then(result => {
  console.log("++++++++++++++++++++++++++++ executeQuery().then(result => {++++++++++++++++++++++++++++++++++++++ ");

  console.log("Query Result:", result);
}).catch(error => {
  console.error("Error in promise:", error);
});
app.on("ready", async (config) => {
    createWindow();
});

//GET data base config
electron.ipcMain.on("get:config", async(event,config) =>{
  const store = new Store();
  ...
  console.log("\\\\\\\\\++++++++++++++++++++++++++++connectToSourceDatabase++++++++++++++++++++++++++++++++++++++\\\\\\\\\\\\\\\ ",await sql.query("SELECT TOP 5 * FROM AllDatas"));
})
...
async function checkForNewDataAndSend(targetUrl) {
  console.log("-*-*-**--*-*-*-*-*-*-*-*-*-*-*-we are in checkForNewDataAndSend  sqlConfig-*-*-*-*-*-**-**-*-*--*-*-*-*-*-*-*-*-*-*-*--*-**-",sqlConfig)
  try {
